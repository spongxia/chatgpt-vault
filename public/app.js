...
function hideMenus() { E.moreMenu.hidden = true; E.exportMenu.hidden = true; }
...
function exportCurrent(format) { if (!S.current) return; const text = format === 'markdown' ? toMarkdown(S.current) : JSON.stringify(S.current, null, 2); const blob = new Blob([text], { type: format === 'markdown' ? 'text/markdown' : 'application/json' }), url = URL.createObjectURL(blob), link = document.createElement('a'); link.href = url; link.download = (S.current.title || 'conversation') + (format === 'markdown' ? '.md' : '.json'); link.click(); setTimeout(function () { URL.revokeObjectURL(url); }, 500); hideMenus(); toast(t('exported', { format: format === 'markdown' ? 'Markdown' : 'JSON' })); }
async function exportAll() { try { const payload = await api('/api/conversations/export'); const url = URL.createObjectURL(new Blob([JSON.stringify(payload.conversations, null, 2)], { type: 'application/json' })), link = document.createElement('a'); link.href = url; link.download = 'conversations.json'; link.click(); setTimeout(function () { URL.revokeObjectURL(url); }, 500); toast(t('exportAllDone', { count: payload.conversations.length })); } catch (error) { toast(t('exportFailed', { message: error.message }), 'error'); } }
...
function bind() {
  ...
  document.onclick = function (e) { if (!e.target.closest('.popover') && !e.target.closest('#more-button') && !e.target.closest('#export-button')) hideMenus(); }; document.onkeydown = function (e) { if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k') { e.preventDefault(); E.search.focus(); } if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'i') { e.preventDefault(); E.importDialog.showModal(); } };
}
