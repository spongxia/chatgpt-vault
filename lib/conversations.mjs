...
export function mergeConversation(existing, incoming) {
  ...
  const localMessages = new Map((existing.messages || []).map(message => [message.id, message]));
  const mergedMessages = new Map(localMessages);
  for (const message of normalized.messages) mergedMessages.set(message.id, message);
  ...
  const messages = (authoritativeRemote
    ? normalized.messages.map(message => {
        ...
        if (!localMessage?.metadata?.vaultEditedAt) return message;
        return {
          ...message,
          content: localMessage.content,
          metadata: {
            ...message.metadata,
            vaultEditedAt: localMessage.metadata.vaultEditedAt
          }
        };
      })
